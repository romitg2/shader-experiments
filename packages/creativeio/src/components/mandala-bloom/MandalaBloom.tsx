...
interface MandalaBloomSimProps {
...
function MandalaBloomSim({
  ...
  useFrame((state, delta) => {
    ...
    // strength forever (this is exactly the "constant glow at rest" bug).
    ...
    if (mat) {
      ...
      mat.uniforms.uSides.value = sides
      mat.uniforms.uRotation.value = state.clock.elapsedTime * rotationSpeed
    }
  })
...
export interface MandalaBloomProps {
  className?: string
  style?: CSSProperties
  ...
  edgeBoost?: number
  /** Number of mirrored wedges the fluid is folded into. */
  sides?: number
  /** Radians per second the fold wedge slowly rotates. */
  rotationSpeed?: number
}
