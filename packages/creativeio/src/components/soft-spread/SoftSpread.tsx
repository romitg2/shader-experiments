...
function SoftSpreadSim({
  ...
  useFrame((_state, delta) => {
    ...
    if (p.active) {
      ...
      splatMat.uniforms.uColor.value.set(dx * 30, dy * 30, 0)
...
export interface SoftSpreadProps {
  className?: string
  style?: CSSProperties
  ...
  velocityDissipation?: number
  /** How quickly the underlying mass fades out each frame (0-1). Slower than the other components by design. */
  densityDissipation?: number
  /** Jacobi iterations for the pressure solve. Higher = more accurate, more expensive. */
  pressureIterations?: number
  /** Bloom color. */
  color?: string
  /** Peak opacity of the bloom (0-1) — kept low by design for a barely-there look. */
  opacity?: number
}

export function SoftSpread({
  className,
  style,
  simResolution = 192,
