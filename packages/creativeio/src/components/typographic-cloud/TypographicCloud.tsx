...
interface TypographicCloudSimProps {
...
function TypographicCloudSim({
  ...
  cellSize,
  color,
}: TypographicCloudSimProps) {
  const { gl } = useThree()
  const atlas = useMemo(() => createGlyphAtlas(CLOUD_LETTERS), [])
    ...
    <mesh ref={displayMeshRef}>
      ...
      <shaderMaterial
        ...
        uniforms={{
          ...
          uLevels: { value: CLOUD_LETTERS.length },
...
export interface TypographicCloudProps {
  className?: string
  style?: CSSProperties
...
export function TypographicCloud({
  className,
  style,
  simResolution = 256,
...
export default TypographicCloud
