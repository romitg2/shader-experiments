...
function LiquidMetalSim({
  ...
  useFrame((_state, delta) => {
    ...
    if (p.active) {
      ...
      splatMat.uniforms.uColor.value.set(0.9, 0.9, 0.9)
      splatMat.uniforms.uRadius.value = 0.0014
    ...
    if (mat) {
      ...
      mat.uniforms.uBaseColor.value.copy(baseColorVec)
...
export interface LiquidMetalProps {
  ...
  velocityDissipation?: number
  /** How quickly the fluid mass fades out each frame (0-1). */
  densityDissipation?: number
  /** Jacobi iterations for the pressure solve. Higher = more accurate, more expensive. */
  pressureIterations?: number
  /** Base metallic tint. */
  color?: string
  /** How strongly the density gradient bends the fake surface normal — higher reads shinier/bumpier. */
  bumpStrength?: number
}
