...
function NeonFlowSim({
  ...
  useFrame((state, delta) => {
    ...
    if (p.active) {
      ...
      splatMat.uniforms.uRadius.value = 0.0016
...
export interface NeonFlowProps {
  className?: string
  style?: CSSProperties
  ...
  velocityDissipation?: number
  /** How quickly the underlying mass fades out each frame (0-1). */
  densityDissipation?: number
  /** Jacobi iterations for the pressure solve. Higher = more accurate, more expensive. */
  pressureIterations?: number
  /** Palette the outline cycles through over time (2+ colors). */
  colors?: string[]
  /** How fast the injected color walks through the palette. */
  colorSpeed?: number
  /** Outline brightness/sharpness multiplier. */
  glow?: number
}

const DEFAULT_COLORS = ['#00f0ff', '#ff00e5', '#39ff14']
