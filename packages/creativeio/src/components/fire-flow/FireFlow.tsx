...
function FireFlowSim({
  ...
  useFrame((_state, delta) => {
    ...
    if (p.active) {
      ...
      // Same velocity splat as Fluid, but with a constant upward bias added
      // on top of the pointer-driven component — flame licks upward even
      // while hovering still, instead of only reacting to drag direction.
      splatMat.uniforms.uTarget.value = velocity.read.texture
      splatMat.uniforms.uPoint.value.set(p.x, p.y)
      splatMat.uniforms.uColor.value.set(dx * 50, dy * 50 + buoyancy, 0)
      ...
      splatMat.uniforms.uColor.value.set(0.85 * energy, 0.85 * energy, 0.85 * energy)
      splatMat.uniforms.uRadius.value = 0.0011
...
export interface FireFlowProps {
  className?: string
  style?: CSSProperties
  ...
  velocityDissipation?: number
  /** How quickly the flame mass fades out each frame (0-1). */
  densityDissipation?: number
  /** Jacobi iterations for the pressure solve. Higher = more accurate, more expensive. */
  pressureIterations?: number
  /** Constant upward velocity injected alongside every splat — higher licks flame upward faster. */
  buoyancy?: number
}

export function FireFlow({
  className,
  style,
  simResolution = 256,
