...
function PixelInkSim({
  ...
  useFrame((state, delta) => {
    ...
    if (p.active) {
      ...
      // Walk along the color palette over time (wrapping), lerping between
      // consecutive stops — a sweeping gesture leaves a trail that shifts
      // through the palette (e.g. green into red) as it flows and blends,
      // same idea as Color Smoke but constrained to a small duotone/tritone
      // set instead of a full hue sweep.
    ...
    if (mat) {
      ...
      mat.uniforms.uMaxFill.value = maxFill
      mat.uniforms.uGrainSpeed.value = grainSpeed
    }
  })
...
export interface PixelInkProps {
  className?: string
  style?: CSSProperties
  ...
  velocityDissipation?: number
  /** How quickly the ink fades out each frame (0-1). */
  densityDissipation?: number
  ...
  colorSpeed?: number
  /** Size (in screen pixels) of each dot/block in the dither grid. */
  pixelSize?: number
  /** How readily a block shows a dot for a given density value — higher = denser ink. */
  dotDensity?: number
  /** Maximum fraction (0-1) of blocks that can ever show a dot — caps peak density so even the core of a trail stays visibly grainy instead of solidifying into a flat fill. */
  maxFill?: number
  /** How many times per second the dot grain re-rolls (higher = more lively twinkle). */
  grainSpeed?: number
}

const DEFAULT_COLORS = ['#22c55e', '#ef4444']

export function PixelInk({
  className,
  style,
  simResolution = 256,
